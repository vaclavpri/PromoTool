...
Public Sub UpdatePrices_Shared(TargetWorkbook As Workbook, SelectedRange As Range, fcType As String, countryCode As String)
    ...
    If rowsToUpdate.Count = 0 Then
        MsgBox "Nebyly vybrány žádný platné řádky!", vbExclamation
        Call LockText(TargetWorkbook)
        Exit Sub
    End If
    
    Debug.Print "Počet řádků k aktualizaci: " & rowsToUpdate.Count
    
   ' 6. SESKUPIT ŘÁDKY PODLE PromoID
    Dim promoGroups As Object
    Set promoGroups = GroupRowsByPromoID(textList, rowsToUpdate)
    
    Debug.Print "Počet unikátních promocí: " & promoGroups.Count
    ...
    For Each promoID In promoGroups.Keys
        ...
        ' Převést string řádků na Collection
        Dim promoRows As Collection
        Set promoRows = StringToCollection(CStr(promoGroups(promoID)))
        
        Debug.Print "  Počet řádků v této promoci: " & promoRows.Count
        
        ' ZMĚNA: Předat fcType a countryCode
    ...
    MsgBox "Aktualizace dokončena:" & vbCrLf & _
           "- Aktualizováno: " & updatedCount & " řádků" & vbCrLf & _
           "- Přidáno: " & addedCount & " řádků" & vbCrLf & _
           "- Smazáno: " & deletedCount & " řádků" & vbCrLf & _
           "- Nenalezeno v PriceList: " & notFoundCount & " řádků", vbInformation
    Exit Sub
...
Private Function GroupRowsByPromoID(textList As Worksheet, rowsToUpdate As Collection) As Object
    ...
    For Each rowNum In rowsToUpdate
        ...
        If promoID <> "" Then
            promoIDKey = promoID  ' Převod na Variant
...
' ===================================================================
' Pomocná funkce - Převod stringu na Collection
' ===================================================================
...
' ===================================================================
' Zpracování jedné promoce (skupina řádků se stejným PromoID)
' ===================================================================
...
For Each productRow In familyProducts
    ...
    If existingProducts.Exists(productFullName) Then
        ' Produkt existuje Ě AKTUALIZOVAT
    ...
    Else
        ...
        Else
            Debug.Print "      Nepřidávám (tVyber = " & vyberValue & ")"
        End If
    End If
Next productRow
...
Private Function AddNewProducts(textList As Worksheet, productsToAdd As Collection, templateRow As Long, promoID As String, countryCode As String) As Long
    ...
    Debug.Print "  fcType pro nový produkty: " & fcType
...
Private Sub CopyFamilySpecificData(textList As Worksheet, targetRow As Long, familyValue As String, promoID As String)
    ...
    Debug.Print "      Kopíruji Family-specific data pro řádek " & targetRow
    
    ' Najít jiný řádek se stejnou Family a PromoID (ale ne targetRow)
    Dim sourceRow As Long
    sourceRow = FindFamilySourceRow(textList, familyValue, promoID, targetRow)
    
    If sourceRow = 0 Then
        Debug.Print "      VAROVÁNÍ: Nenalezen zdrojový řádek pro Family " & familyValue
        Exit Sub
    End If
    
    Debug.Print "      Zdrojový řádek: " & sourceRow
    ...
    ' tPriceType je hodnota (ne vzorec), kopírovat normálně
...
' ===================================================================
' Kopírování vzorce s úpravou čísla řádku
' ===================================================================
Private Sub CopyFormulaToRow(ws As Worksheet, sourceRow As Long, targetRow As Long, rangeName As String)
    ...
    ' Zkontrolovat, zda zdrojový buňka obsahuje vzorec
    If sourceCell.HasFormula Then
        ' Zkopírovat vzorec a upravit číslo řádku
        Dim originalFormula As String
        Dim adjustedFormula As String
        
        originalFormula = sourceCell.formula
        
        ' Nahradit všechny výskyty zdrojovýho řádku cílovým řádkem
        ...
        Debug.Print "          Původní: " & originalFormula
...
Private Function ReplaceRowReferences(formula As String, oldRow As Long, newRow As Long) As String
    ...
    ' Dvoumástn� sloupce (AA-AZ, BA-BZ, CA-CZ)
...
' ===================================================================
' Najít zdrojový řádek se stejnou Family a PromoID
' ===================================================================
Private Function FindFamilySourceRow(textList As Worksheet, familyValue As String, promoID As String, excludeRow As Long) As Long
    ...
    Dim i As Long
    For i = 3 To lastRow  ' Začít od řádku 3 (přeskočit header)
        If i <> excludeRow Then  ' Přeskočit cílový řádek
...
Private Function DeleteProducts(textList As Worksheet, rowsToDelete As Collection) As Long
    ...
    ' Seřadit řádky od nejvyššího k nejnižšímu (aby se při mazání neposouvaly indexy)
