...
Public Sub SAP_CRM(TargetWorkbook As Workbook, SelectedRange As Range)
    ...
    If productsCol.Count = 0 Then
        MsgBox "žádný data k načtení z PriceList!", vbExclamation
        Call LockText(TargetWorkbook)
        Call LockSAP(TargetWorkbook)
        Exit Sub
    End If
    ...
    ' Načíst všechna data z výběru najednou do pole
    ...
    ' Vymazání dat z listu SAP (zachovat hlavičky na řádcích 1-3)
    ...
    ' Příprava dat pro SAP (nový formát)
    ...
    For j = 1 To tRowCount
        ...
        ' Najít odpovúdajůc� produkt v kolekci
            ...
            If productRow.Exists("Family") And productRow.Exists("material_name") And productRow.Exists("volume_l") Then
                ...
                If textKey = productKey Then
                    ...
                    If productRow.Exists("base_price") And productRow.Exists("special_discount") Then
                        ...
                        ' Nastavení hodnoty s desetinnou tečkou a záporným znamánkem
    ...
    ' Bulk zápis do SAP (NOVÉ FORMáT) - začíná od řádku 4
    If sapRowIndex > 1 Then
        Dim sapStartRow As Long
        sapStartRow = 4 ' Začínáme od 4. řádku (řádky 1-3 jsou hlavičky)
    ...
    Debug.Print "CSV: Zpracováno " & (sapRowIndex - 1) & " SAP řádků a " & (crmRowIndex - 1) & " CRM řádků"
...
Public Sub ExportData(TargetWorkbook As Workbook)
    ...
    ' Převod číselných datumá na textové formát
    Dim lastRow As Long
    Dim i As Long
    lastRow = Wbk.Sheets(1).UsedRange.rows.Count
    
    ' Převod dat ve sloupcůch F a G na čitelné textové formát
    For i = 2 To lastRow
        ' Sloupec F (PLATOD)
    ...
    ' Odstranění všech tlačítek z nového listu
    Dim shp As Shape
    ...
    ElseIf path2 <> "" And fso.FolderExists(path2) Then
        ...
        If Right(targetFolderPath, 1) <> "\" Then targetFolderPath = targetFolderPath & "\"
    Else
        MsgBox "žádný z cest v Settings (B7, B8) neexistuje!", vbCritical
        GoTo CleanUp
    End If
    ...
    ' Uložení do lokálně složky
    Wbk.SaveAs _
        fileName:=wbkLocalPath, _
        FileFormat:=xlOpenXMLWorkbook, _
        CreateBackup:=False
    
    Debug.Print "Export dokonížen: " & wbkLocalPath
    
    If sharePointSaved Then
        MsgBox "Export dokonížen!" & vbCrLf & _
               "Lokálně: " & wbkLocalPath & vbCrLf & _
               "SharePoint: " & wbkSharePointPath, vbInformation
    Else
        MsgBox "Export dokonížen lokálně!" & vbCrLf & _
               "Lokálně: " & wbkLocalPath & vbCrLf & vbCrLf & _
